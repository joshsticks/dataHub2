define([
	...
	var HitTheNoteView = Backbone.View.extend({
		...
		render: function () {
			...
			$.ajax({
					...
					{
					      return $(this).attr("Note")+ $(this).attr("Octave");
					}) ;
					...
					var myScore = $(response).find("ExhibitSession HitThatNote").attr("Note") +  $(response).find("ExhibitSession HitThatNote").attr("Octave");
					var meIndex = $.inArray( myScore, labelArray);
					labelArray[meIndex] = "My Note " + labelArray[meIndex];
					$("#yourScore").text("My Note " + myScore + "");
