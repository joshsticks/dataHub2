	...
	var CreativityView = Backbone.View.extend({
		...
		render: function () {
			...
			$.ajax({
					...
					{
					      return $(this).attr("CorrectPieces");
					}) ;
					...
					var myScore = parseFloat($(response).find("ExhibitSession ConveyorBelt").attr("CorrectPieces")).toFixed(0);
					var meIndex = $.inArray( myScore, labelArray);
					labelArray[meIndex] = "My Score " + labelArray[meIndex];
					$("#yourScore").text("My Score " + myScore + " Correct Pieces");
